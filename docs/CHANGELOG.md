...
- `[Lookup]` Fixed a bug with settings async server-side and single select modes.  The grid was not deselecting the previously-selected value when a new row was clicked.  If the value is pre-selected in the markup, the lookup modal will no longer close prematurely. ([PR#1654](https://github.com/infor-design/enterprise/issues/1654))
