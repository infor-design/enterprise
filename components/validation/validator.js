...
Validator.prototype = {
  ...
  validate: function (field, showTooltip, e) {
    ...
    for (var props in Validation.ValidationTypes) {
      validationType = Validation.ValidationTypes[props];
      self.removeMessage(field, validationType.type);
  ...
  addMessage: function(field, message, type, inline, showTooltip, isAlert) {
    ...
    var loc = this.getField(field),
       ...
       appendedMsg = message,
       validationType = Validation.ValidationTypes[type] || Validation.ValidationTypes.error;
