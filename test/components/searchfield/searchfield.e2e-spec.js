...
if (utils.isChrome() && utils.isCI()) {
  ...
  describe('Searchfield close icon placement tests', () => {
    beforeEach(async () => {
      await utils.setPage('/components/field-options/example-index?theme=classic&mode=light&layout=nofrills');
      await browser.driver
        .wait(protractor.ExpectedConditions
          .presenceOf(element(by.id('example-searchfield-clearable'))), config.waitsFor);
    });
    ...
    if (utils.isChrome() && utils.isCI()) {
      it('should not visual regress', async () => {
        ...
        await browser.driver
        .wait(protractor.ExpectedConditions.presenceOf(searchfieldClearable), config.waitsFor);
        const compactButton = await element(by.id('toggle-compact'));
        await browser.driver
        .wait(protractor.ExpectedConditions.presenceOf(compactButton), config.waitsFor);
