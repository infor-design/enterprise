  ...
  $.fn.searchfield = function(options) {
    ...
    SearchField.prototype = {
      ...
      build: function() {
        ...
        if (!icon || !icon.length) {
          icon = $.createIconElement('search').insertAfter(this.element);
        }
        ...
        if (this.hasCategories()) {
          ...
          if (!ddIcon.length) {
            ddIcon = $.createIconElement({ classes: 'icon-dropdown', icon: 'dropdown' });
          }
        ...
        if (this.settings.clearable) {
          this.xButton = $.createIconElement({ classes: 'close', icon: 'close' });
