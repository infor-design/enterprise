    ...
    Plugin.prototype = {
      ...
      val: function(value) {
        ...
        for (i = 0; i < this.allInputs.length; i++) {
          ...
          //Handle Half Star
          input.next(svgSelector).find('use').attr('xlink:href', $.getSvgIconLink('#icon-star-filled'));
          ...
          if (chkIdx !== self.currentValue && i+1 === chkIdx) {
           input.addClass('is-half').next(svgSelector).find('use').attr('xlink:href', $.getSvgIconLink('#icon-star-half'));
          }
        }
        if (chkIdx <= 0) {
