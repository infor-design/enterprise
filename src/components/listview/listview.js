  ...
  */
  handleResize() {
    ...
    if (this.element.hasClass('card-list')) {
      const cards = $('li .card .card-buttonset', this.element);
      ...
      const addMore = (element, len) => {
        const children = $(element).children(':visible');
        ...
        if ($(element).hasClass('is-overflowed')) {
          ...
          const more = $('<button class="btn-actions" type="button"></button>')
            .html(`${$.createIcon({ icon: 'more' })
            }<span class="audible">${Locale.translate('MoreActions')}</span>`)
            .attr('title', Locale.translate('More'));
          container.append(more);
      ...
      if (shouldOverflow) {
        cards.each((index, element) => {
          addMore(element, $(element).children(':visible').length);
        });
      } else {
        cards.each((index, element) => {
          if ($(element).children(':visible').length > 2) {
            addMore(element, $(element).children(':visible').length - 2);
          } else if ($(element).hasClass('is-overflowed')) {
            const more = $(element).find('.card-content-action button');

            if (more) {
              const parent = more.parent();
              more.data('popupmenu').destroy();
              more.remove();
              parent.remove();
            }

            $(element).removeClass('is-overflowed');
          }
        });
      }
    }
