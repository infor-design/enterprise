...
MonthView.prototype = {
  ...
  appendMonthYearPicker(month, year) {
    ...
    monthList += `<li class="picklist-item down">
        <a tabindex="0"${isRippleClass}>
          <span class="audible">${Locale.translate('NextMonth')}</span>
    ...
    yearList += `<li class="picklist-item down">
        <a tabindex="0"${isRippleClass}>
          <span class="audible">${Locale.translate('NextYear')}</span>
          <svg class="icon" focusable="false" aria-hidden="true" role="presentation">
            <use href="#icon-caret-down"></use>
          </svg>
        </a>
  ...
  handleMonthYearPane() {
    ...
    const appendMonth = (upDown) => {
      ...
      const month = parseInt(monthList[(upDown === 'up' ? 1 : monthLen)].querySelector('a').getAttribute('data-month'), 10);
      const monthContainerJQ = $(monthContainer);

      const selected = monthContainerJQ.find('li.is-selected');
      const index = selected.index();

      selected.removeClass('is-selected');

      const top = monthList[1];
      const bottom = monthList[monthLen];
      const li = $('<li class="picklist-item"></li>');

      if (upDown === 'up') {
        const idx = month === 0 ? 11 : month - 1;
        const a = `<a href="#" tabindex="-1" data-month="${idx}"${isRippleClass}>${wideMonths[idx]}</a>`;
        li.append(a);
        bottom.remove();
        li.insertBefore(top);
      } else {
        const idx = month === 11 ? 0 : month + 1;
        const a = `<a href="#" tabindex="-1" data-month="${idx}"${isRippleClass}>${wideMonths[idx]}</a>`;
        li.append(a);
        top.remove();
      ...
      if (!s.inPage) {
        $(monthContainerJQ.find('li.picklist-item').get(index))
  ...
  */
  ...
  /**
   * Focus a specific date visually.
   * @private
   * @param {date | string} date specific date or a date key (hash string of the date)
  */
  focusDay() {
  ...
  /**
   * Checks if the month has the current date and highlights it.
   */
  setCurrentDay() {
    let todayDate = new Date();
    if (!this.isIslamic && typeof date !== 'string') {
      todayDate = stringUtils.padDate(
  ...
  handleKeys() {
    ...
    this.element.off('keydown.monthview').on('keydown.monthview', '.monthview-table', (e) => {
      ...
      if (key === 40) {
        handled = true;
        if (!this.validateDisplayRange(dateUtils.add(this.focusDate, 7, 'days'))) return false;
        if (s.range.useRange) {
      ...
      if (key === 38) {
        handled = true;
        if (!this.validateDisplayRange(dateUtils.subtract(this.focusDate, 7, 'days'))) return false;
        if (s.range.useRange) {
          idx = allCell.index(e.target) - 7;
          selector = allCell.eq(idx);
          if (idx > -1) {
          ...
          if (this.focusDate.getMonth() > minDate.getMonth()) {
            this.focusDate.setDate(this.focusDate.getDate() - 7);
