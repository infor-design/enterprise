        ...
        if (self.settings.useLine) {
          if (dataset.length === dataset.map(d => d.line).length) {
            ...
            lineGroup.append('path')
              ...
              .attr('class', 'line')
              .style('opacity', 0)
              .attr('stroke', classicDark ? '#BDBDBD' : newDark ? '#CACACE' : theme.new && theme.currentTheme.modeId !== 'dark' ? '#4A4A4F' : '#313236')
              .attr('stroke-width', 2)
              .attr('fill', 'none');
            ...
            if (!self.settings.hideDots) {
              lineGroup.selectAll('circle')
                ...
                .attr('r', 5)
                .style('fill', classicDark ? '#BDBDBD' : newDark ? '#CACACE' : theme.new && theme.currentTheme.modeId !== 'dark' ? '#4A4A4F' : '#313236')
